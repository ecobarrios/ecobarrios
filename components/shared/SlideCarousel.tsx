          ...
          {imagesSlide.map((i) => (
            <CarouselItem key={i.id}>
              <Card className="rounded-none shadow-none w-full bg-transparent">
                <div
                  className="flex w-full md:min-h-[60vh] md:max-h-[60vh] h-[300px]  md:bg-cover bg-center bg-contain bg-no-repeat items-center justify-center"
                  style={{ backgroundImage: `url(${i.src.src})`,width: "100%" }}
                />
              </Card>
            </CarouselItem>
          ))}
